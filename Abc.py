...
class Student:
    def __init__(self,root):
        ...
        lbl_category.grid(row=3,column=0,sticky=W,padx=2,pady=7)
        ...
        com_txt_category.grid(row=3,column=1,padx=2,pady=7)
        ...
        self.student_table=ttk.Treeview(table_frame,column=("class","stream","session","course","adm_no","name","section","roll_no","gender","dob","category","phone_no"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    #Report Card
        mycon=mys.connect(host='localhost',user='root',passwd='Moonknight',database='school')
        mycursor=mycon.cursor()
        mycursor.execute('''create table if not exists report(Roll_no int primary key,
                                                             Adm_no int,
                                                             Name varchar(45),
                                                             total_marks int,
    ...
    #Entering into Report card
    def Report_enter(self):
        global e1,e2,e3,e4,e5,e6,e7
        root1=Tk()
        label=Label(root1,text="Welcome",font='arial 25 bold')
        label.pack()
        frame=Frame(root1,height=500,width=400)
        frame.pack()
        l1=Label(root1,text="ENTER ROLL NO WHOSE REPORT HAS TO BE CREATED")
        l1.place(x=10,y=130)
        e1=Entry(root1)
        e1.place(x=200,y=130)
        l2=Label(root1,text="ENTER NAME OF THE STUDENT")
        l2.place(x=10,y=170)
        e2=Entry(root1)
        e2.place(x=200,y=170)
        l3=Label(root1,text="MATHS MARKS")
        l3.place(x=10,y=210)
        e3=Entry(root1)
        e3.place(x=200,y=210)
        l4=Label(root1,text="PHYSICS MARKS")
        l4.place(x=10,y=250)
        e4=Entry(root1)
        e4.place(x=200,y=250)
        l5=Label(root1,text="CHEMISTRY MARKS")
        l5.place(x=10,y=290)
        e5=Entry(root1)
        e5.place(x=200,y=290)
        l6=Label(root1,text="CS MARKS")
        l6.place(x=10,y=330)
        e6=Entry(root1)
        e6.place(x=200,y=330)
        l7=Label(root1,text="ENGLISH MARKS")
        l7.place(x=10,y=370)
        e7=Entry(root1)
        e7.place(x=200,y=370)
        b1=Button(root1,text="SUBMIT",command=self.Submit_1())
    ...
    def Submit_1(self):
        ...
        p6=e6.get()
        p7=e7.get()
        p8=int(p3)+int(p4)+int(p5)+int(p6)+int(p7)
        p9=int(p8/5)
        mycon=mys.connect(host='localhost',user='root',passwd='Moonknight',database='school')
        mycursor=mycon.cursor()
        mycursor.execute("insert into report values(%s,%s,%s,%s)",(p1,p2,p8,p9,))
        mycon.commit()
        messagebox.showinfo("Success")

    #TO DISPLAY THE REPORT CARD
    def Report_display():
        root4=Tk()
        l=[]
        m=[]
