...
class Student:
    def __init__(self,root):
        ...
        #left frame 
        dataleftframe=LabelFrame(manage_frame,bd=4,relief=RIDGE,padx=2,font=("times new roman",12,"bold"),fg="maroon",bg="white")
        ...
        #Report Card
        repcd=LabelFrame(dataleftframe,bd=4,relief=RIDGE,padx=2,text="Student Report Card",font=("times new roman",12,"bold"),fg="maroon",bg="white")
        repcd.place(x=0,y=290,width=650,height=215)
        
        #Labels Entry
        #Maths/Hindi marks
        lbl_maths=Label(repcd,font=("helvetica",11,"bold"),text="Maths/Hindi Marks:",bg="white")
        lbl_maths.grid(row=0,column=0,sticky=W,padx=2,pady=7)

        txt_maths=ttk.Entry(repcd,textvariable=self.var_mathsmarks,width=22,font=("helvetica",11,"bold"))
        txt_maths.grid(row=0,column=1,padx=2,pady=7)
        
        #Physics/Accountancy marks
        lbl_phy=Label(repcd,font=("helvetica",11,"bold"),text="Physics/Accountancy Marks:",bg="white")
        lbl_phy.grid(row=1,column=0,sticky=W,padx=2,pady=7)

        txt_phy=ttk.Entry(repcd,textvariable=self.var_phymarks,width=22,font=("helvetica",11,"bold"))
        txt_phy.grid(row=1,column=1,padx=2,pady=7)
        
        #Chemistry/Economics marks
        lbl_chem=Label(repcd,font=("helvetica",11,"bold"),text="Chemistry/Economics Marks:",bg="white")
        lbl_chem.grid(row=2,column=0,sticky=W,padx=2,pady=7)

        txt_chem=ttk.Entry(repcd,textvariable=self.var_chemmarks,width=22,font=("helvetica",11,"bold"))
        txt_chem.grid(row=2,column=1,padx=2,pady=7)
        
        #CS/Biology/Business studies marks
        lbl_cs=Label(repcd,font=("helvetica",11,"bold"),text="Computer Sc./Bio/Business Studies Marks:",bg="white")
        lbl_cs.grid(row=3,column=0,sticky=W,padx=2,pady=7)

        txt_cs=ttk.Entry(repcd,textvariable=self.var_csmarks,width=22,font=("helvetica",11,"bold"))
        txt_cs.grid(row=3,column=1,padx=2,pady=7)
        
        #English marks
        lbl_eng=Label(repcd,font=("helvetica",11,"bold"),text="English Marks:",bg="white")
        lbl_eng.grid(row=4,column=0,sticky=W,padx=2,pady=7)

        txt_eng=ttk.Entry(repcd,textvariable=self.var_engmarks,width=22,font=("helvetica",11,"bold"))
        ...
        mycursor.execute('''create table if not exists student(Class varchar(10) NOT NULL,
                                                               ...
                                                               CSorBiologyorBusiness_studies_marks int DEFAULT NULL,
                                                               English_marks int DEFAULT NULL,Total_marks int NOT NULL,
        ...
        self.student_table=ttk.Treeview(table_frame,column=("class","stream","session","course","adm_no","name","section","roll_no","gender","dob","category","phone_no","mathsorhindi_marks","physicsoraccountancy_marks","chemistryoreconomics_marks","csorbiologyorbusinness_studies_marks","english_marks","total_marks","total_percentage"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.student_table.heading("physicsoraccountancy_marks",text="Physics/Accountancy Marks")
        self.student_table.heading("chemistryoreconomics_marks",text="Chemistry/Economics Marks")
        self.student_table.heading("csorbiologyorbusinness_studies_marks",text="Computer Sc./Biology/Business Studies Marks")
    ...
    def add_data(self):
        if(self.var_class.get()=="" or self.var_admno.get()==""):
            ...
            try:
                p6=int(p1)+int(p2)+int(p3)+int(p4)+int(p5)
                p7=int(p6/5)
                mycon=mys.connect(host='localhost',user='root',passwd='Moonknight',database='school')
                mycursor=mycon.cursor()     
        ...
        if (self.var_class.get()=="" or self.var_admno.get()==""):
            ...
            try:
                ...
                if update>0:
                    mycon=mys.connect(host='localhost',user='root',password='Moonknight',database='school')
                    mycursor=mycon.cursor()     
                    mycursor.execute("update student set Class=%s,Stream=%s,Session=%s,Course=%s,Adm_no=%s,Name=%s,Section=%s,Roll_no=%s,Gender=%s,DOB=%s,Category=%s,Phone_no=%s,MathsorHindi_marks=%s,PhysicsorAccountancy_marks=%s,ChemistryorEconomics_marks=%s,CSorBiologyorBusiness_studies_marks=%s,English_marks=%s,p6=%s,p7=%s where Adm_no=%s",
                                     (self.var_class.get(),               
                                     self.var_stream.get(),
                                     self.var_session.get(),
                                     self.var_course.get(),
